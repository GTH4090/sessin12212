...
public partial class MainMenu : UserControl
{
    private void LoadData()
    {
        try
        {
            List<Visit> visits = Db.Visits.Where(el => el.UserId == Log.Id).Include(el => el.Employee).ThenInclude(el =>
                el.Division).Include(el => el.Type).Include(el =>
                el.Status).Include(el => el.Target).ToList();
            if (visits.Count() != 0)
            {
